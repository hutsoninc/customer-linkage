...
- [ ] **EQUIP entity ID write-back — confirm no-overwrite behavior (Phase 1.2 natural test):** The Help Document confirms linkages ARE sent back to EQUIP overnight. However, WILSONSOAK71156 still shows the old stale entity ID after the test upload — suggesting the tool won't overwrite an existing `Ckc_Id`, only populate a blank one. The Phase 1.2 records (all have `Ckc_Id = NULL` in EQUIP) are the natural test: after Phase 1.2 upload and overnight sync, run `queries/phase-1/block-7c.sql` to confirm whether `Ckc_Id` was populated. If confirmed no-overwrite, records that linked to a merged entity will need their stale `Ckc_Id` corrected manually or via a JDIS/Customer Linkage team ticket.
